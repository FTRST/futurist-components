...
const windowStyle = {
    default: css`
        ...
        left: 1em;
        padding: 0;
        border: .25em double #6BF178;
        background-color: rgba(2,17,27,.7);
    `
};
    ...
    return (
        windowDetails ? (
            <>
                ...
                >
                    <StyledWindow style={{ zIndex: windowDetails.zIndex }} additionalStyle={style}>
                        <Resizable
                        ...
                        >
                            <TitleBar 
                                ...
                                expandAction={() => resizeWindow(manipulateWindows, id, {width: device.width, height: device.height, maximize: true, prevWidth: windowDetails.width, prevHeight: windowDetails.height})} 
                                closeAction={() => closeWindow(manipulateWindows, id)} 
                                minimizeAction={() => resizeWindow(manipulateWindows, id, {width: windowDetails.preveWidth, height: windowDetails.prevHeight, maximize: false})}
                                maximize={windowDetails.maximize}
                            />
                            <StyledWindowBackground style={{ height: modalBackgroundHeight, width: modalBackgroundWidth}}>
